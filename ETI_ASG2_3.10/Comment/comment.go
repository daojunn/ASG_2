...
func getAllStudents(db *sql.DB) []Object {
	url := "http://localhost:9135/api/student"
...
func getAllTutors(db *sql.DB) []Object {
	url := "http://localhost:9136/api/tutor"
	response, err := http.Get(url)
...
func tutorReceivedComments(w http.ResponseWriter, r *http.Request) {
	db, err := sql.Open("mysql", "root:password@tcp(commentdb:3306)/commentdb")
	if err != nil {
		panic(err.Error())
	}
	params := mux.Vars(r)
...
func getAllTutorComments(db *sql.DB) []Comment {
	...
	query := fmt.Sprintf("SELECT * FROM Comment WHERE CreatorType = 'Tutor'")
...
func getAllStudentComments(db *sql.DB) []Comment {
	...
	query := fmt.Sprintf("SELECT * FROM Comment WHERE CreatorType = 'Student'")
...
func main() {
	...
	fmt.Println("Listening at port 9125")
	log.Fatal(http.ListenAndServe(":9125", handlers.CORS(headers, origins, methods)(router)))
}
