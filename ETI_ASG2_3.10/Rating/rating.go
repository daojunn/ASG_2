...
func rating(w http.ResponseWriter, r *http.Request) {
	db, err := sql.Open("mysql", "user2:password@tcp(127.0.0.1:3306)/ETIAssignment2Rating")
	if err != nil {
		panic(err.Error())
	}
	if r.Method == "POST" {
...
func getAllStudentRatings(db *sql.DB) []Rating {
	...
	query := fmt.Sprintf("SELECT * FROM ETIAssignment2Rating.Rating WHERE CreatorType = 'Student'")
